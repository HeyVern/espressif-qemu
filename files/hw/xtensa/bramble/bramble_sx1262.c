...
struct BrambleSx1262State {
    ...
    uint16_t irq_status;
    /* Latched by SetPacketType; RadioLib reads it back and rejects a wrong modem. */
    uint8_t packet_type;   /* pending IRQ flags (driven by emu-link txdone/rx) */
...
static uint32_t bramble_sx1262_transfer(SSIPeripheral *dev, uint32_t val)
{
    ...
    if (idx == 0) {
        { static uint8_t seen[256]; if (!seen[in]) { seen[in] = 1;
            qemu_log("SX newop=0x%02x\n", (unsigned)in); } }
        s->opcode = in;
        s->reg_addr = 0;
        s->buf_offset = 0;
    ...
    case SX1262_CMD_READ_REGISTER:
        if (idx == 1) {
            ...
            { static unsigned rn; if (rn++ < 24)
                qemu_log("RDREG addr=0x%04x idx=%u -> 0x%02x\n",
                         (unsigned)s->reg_addr, (unsigned)idx, (unsigned)out); }
            s->reg_addr++;
        }
        break;
    ...
    { static unsigned on; if (on++ < 44)
        qemu_log("OUT idx=%u in=0x%02x out=0x%02x\n",
                 (unsigned)idx, (unsigned)in, (unsigned)out); }
    return out;
}
...
static void bramble_sx1262_realize(SSIPeripheral *dev, Error **errp)
{
     ...
     * does not name the part. Real silicon ships it; a zeroed register file does not. */
    {
        ...
        memcpy(st->regs + 0x0320, "SX1261", 6);   /* real silicon reports SX1261 for both parts */
    }
    BrambleSx1262State *s = BRAMBLE_SX1262(dev);
    (void)errp;
