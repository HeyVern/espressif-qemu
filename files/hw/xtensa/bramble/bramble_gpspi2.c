...
static void bramble_gpspi2_transfer(BrambleGpspi2State *s)
{
    ...
    { static unsigned tn; if (tn++ < 8)
        qemu_log("XFER mosi=%d miso=%d bytes=%u dma_tx=%d dma_rx=%d\n",
                 (int)do_mosi, (int)do_miso, (unsigned)data_bytes,
                 (int)dma_tx, (int)dma_rx); }
...
static void bramble_gpspi2_probe(void)
{
    static unsigned n;
    if (n++ == 0) qemu_log("GPSPI2 first guest write\n");
    if (n % 5000 == 0) qemu_log("GPSPI2 writes=%u\n", n);
}

static void bramble_gpspi2_write(void *opaque, hwaddr addr, uint64_t value,
                                 unsigned int size)
{
    bramble_gpspi2_probe();
    BrambleGpspi2State *s = BRAMBLE_GPSPI2(opaque);
                           ...
                           DeviceState *intc)
{
    ...
    s_gpspi2_for_cs = s;
    bramble_gpio_set_out_observer(bramble_gpspi2_cs_observer);
